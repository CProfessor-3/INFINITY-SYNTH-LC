...
class chrome_render:
    ...
    def get_remaining_space(self) -> float:
        """Returns px gap between .main_content bottom and .page_bottom top (or a4-page bottom)."""
        return self.driver.execute_script('''
            const main = document.querySelector('.main_content');
            const footer = document.querySelector('.page_bottom');
            if (!main) return 0;
            const mainBottom = main.getBoundingClientRect().bottom;
            const ref = footer
                ? footer.getBoundingClientRect().top
                : document.querySelector('.a4-page').getBoundingClientRect().bottom;
            return Math.max(0, ref - mainBottom);
        ''')
